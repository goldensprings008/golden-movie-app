...
const INTERNET_ARCHIVE_SEARCH_URL =
  "https://archive.org/advancedsearch.php";

// Searches Internet Archive's public-domain feature-film collection.
export async function searchFreeMovies(page = 1) {
  ...
  const response = await fetch(
    `${INTERNET_ARCHIVE_SEARCH_URL}?${parameters.toString()}`
  );

  if (!response.ok) {
    throw new Error("Unable to load free movies. Please try again.");
  }

  return response.json();
}

// Retrieves available files for one Internet Archive movie item.
export async function getFreeMovieFiles(identifier) {
  const response = await fetch(
    `https://archive.org/metadata/${encodeURIComponent(identifier)}`
  );

  if (!response.ok) {
    throw new Error("Unable to load this movie. Please try again.");
  }
